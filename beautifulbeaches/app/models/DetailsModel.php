...
class DetailsModel
{
    ...
    public function getMoreInfoByIds($id)
    {
        try {
            $sql = "
                    SELECT * FROM beach_more_info_view
                    WHERE id = :id";
            $stmt = $this->__conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();
    ...
    public function saveReviews($starValue, $beach_id, $name, $comment)
    {
        try {
            ...
            if ($stmt->execute()) {
                $reviewId = $this->__conn->lastInsertId();
                $beachId = $beach_id;
                if (!isset($_SESSION["reviews"]["idReviewAndBeach"])) {
                    $_SESSION["reviews"]["idReviewAndBeach"] = array();
                }
                $_SESSION["reviews"]["idReviewAndBeach"][] = array("reviewId" => $reviewId, "beachId" => $beachId);
            } else {
                echo "erro" . $sql;
            }
        } catch (PDOException $e) {
    ...
    public function getFlightsbyid($id) {
        try{
            if(isset($this->__conn)){
                $sql = "select * from beach_flight_view where id=:id";
                $stmt = $this->__conn->prepare($sql);
                $stmt -> bindParam("id", $id, PDO::PARAM_INT);
                $stmt -> execute();
                $result = $stmt -> fetchAll(PDO::FETCH_ASSOC);
